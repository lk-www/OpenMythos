...
# A is stored as a 1-D diagonal vector (shape: (dim,)) for memory efficiency.
# For diagonal matrices, eigenvalues == diagonal entries, so spectral radius
# is simply max(|A_i|). No need to call torch.linalg.eigvals (which requires 2D).
rho = A.abs().max().item()
print(
